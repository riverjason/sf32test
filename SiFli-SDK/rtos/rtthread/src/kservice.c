...
__ROM_USED void rt_assert_handler(const char *ex_string, const char *func, rt_size_t line)
{
...

    {
...
#endif
        {
            ...
            /* Raw output first: print only addresses to avoid bad-string dereference. */
            {
                char assert_msg[160];
                rt_snprintf(assert_msg, sizeof(assert_msg),
                            "\r\n[ASSERT_RAW] line=%d func_ptr=0x%08x expr_ptr=0x%08x\r\n",
                            (int)line, (unsigned int)func, (unsigned int)ex_string);
                rt_hw_console_output(assert_msg);
            }

#if defined(RT_USING_ULOG)
            /* In ISR context, reopening console device may trigger nested asserts. */
            if (rt_interrupt_get_nest() == 0)
            {
            ...
            rt_kprintf("Assertion failed: line=%d func_ptr=0x%08x expr_ptr=0x%08x\n",
                       (int)line, (unsigned int)func, (unsigned int)ex_string);
            rt_kprintf("Previous ISR enable %x\n", level);

            rt_show_sys_info();
            rt_kprintf("[ASSERT_SUMMARY] line=%d func_ptr=0x%08x expr_ptr=0x%08x prev_isr=0x%08x\n",
                       (int)line, (unsigned int)func, (unsigned int)ex_string, (unsigned int)level);
