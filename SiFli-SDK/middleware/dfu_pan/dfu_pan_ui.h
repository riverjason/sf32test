 ...
 * 
 * Enumeration of all possible message types that can be sent to the OTA UI task
 * to update various UI elements during the OTA process.
 */
typedef enum 
{
    ...
    UI_MSG_UPDATE_BUTTON_COLOR,        /**< Update button background color */
    UI_MSG_SHOW_FAILURE_POPUP,         /**< Show OTA failure popup */
    UI_MSG_SHOW_SUCCESS_POPUP,         /**< Show OTA success popup */
    UI_MSG_SHOW_NETWORK_ERROR_POPUP,   /**< Show network error popup */
    UI_MSG_SHOW_VERSION_CONFIRM,       /**< Show version confirmation dialog */
    UI_MSG_START_OTA_UPDATE            /**< Start OTA update process */
} ui_msg_type_t;
 ...
 * 
 * Initialize LittlevGL graphics library, create UI interface and message queue,
 * and handle UI update messages from other modules.
 * Including Bluetooth connection status, network status, OTA progress updates, etc.
 * 
 ...
 * 
 * @param type Message type (such as Bluetooth status update, network status update, progress update, etc.)
