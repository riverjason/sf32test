...
#define TC_JPEG_DEC_LOG_I LOG_I
#define TC_JPEG_DEC_LOG_E LOG_E
#define TC_JPEG_PP_LOG_I LOG_I
#define  DBG_LEVEL            DBG_ERROR  //DBG_INFO  //DBG_LOG //
#undef LOG_TAG
#define LOG_TAG                "TC.JPEG.DEC"
#include "log.h"
...
static void PrintGetImageInfo(JpegDecImageInfo *imageInfo)
{
    ...
    {
        /* decode thumbnail */
        TC_JPEG_DEC_LOG_I("\t-JPEG THUMBNAIL IN STREAM\n");
        TC_JPEG_DEC_LOG_I("\t-JPEG THUMBNAIL INFO\n");
        TC_JPEG_DEC_LOG_I("\t\t-JPEG thumbnail width: %d\n",
                            imageInfo->outputWidthThumb);
        TC_JPEG_DEC_LOG_I("\t\t-JPEG thumbnail height: %d\n",
        ...
        case JPEGDEC_BASELINE:
            TC_JPEG_DEC_LOG_I("\t\t-JPEG: STREAM TYPE: JPEGDEC_BASELINE\n");
            break;
        case JPEGDEC_PROGRESSIVE:
            TC_JPEG_DEC_LOG_I("\t\t-JPEG: STREAM TYPE: JPEGDEC_PROGRESSIVE\n");
            break;
        case JPEGDEC_NONINTERLEAVED:
            TC_JPEG_DEC_LOG_I("\t\t-JPEG: STREAM TYPE: JPEGDEC_NONINTERLEAVED\n");
            break;
        }
    ...
    TC_JPEG_DEC_LOG_I("\t-JPEG FULL RESOLUTION INFO\n");
    TC_JPEG_DEC_LOG_I("\t\t-JPEG width: %d\n", imageInfo->outputWidth);
    TC_JPEG_DEC_LOG_I("\t\t-JPEG height: %d\n", imageInfo->outputHeight);
    ...
    if (imageInfo->thumbnailType == JPEGDEC_THUMBNAIL_JPEG)
    {
        TC_JPEG_DEC_LOG_I("\t-JPEG ThumbnailType: JPEG\n");
#ifdef ASIC_TRACE_SUPPORT
        trace_jpegDecodingTools.thumbnail = 1;
#endif
    }
    else if (imageInfo->thumbnailType == JPEGDEC_NO_THUMBNAIL)
        TC_JPEG_DEC_LOG_I("\t-JPEG ThumbnailType: NO THUMBNAIL\n");
    else if (imageInfo->thumbnailType == JPEGDEC_THUMBNAIL_NOT_SUPPORTED_FORMAT)
        TC_JPEG_DEC_LOG_I("\t-JPEG ThumbnailType: NOT SUPPORTED THUMBNAIL\n");
}
...
static void PrintJpegRet(JpegDecRet *pJpegRet)
{
    ...
    case JPEGDEC_FRAME_READY:
        TC_JPEG_DEC_LOG_I("TB: jpeg API returned : JPEGDEC_FRAME_READY\n");
        break;
    case JPEGDEC_OK:
        TC_JPEG_DEC_LOG_I("TB: jpeg API returned : JPEGDEC_OK\n");
        break;
    case JPEGDEC_ERROR:
        TC_JPEG_DEC_LOG_E("TB: jpeg API returned : JPEGDEC_ERROR\n");
        break;
    case JPEGDEC_DWL_HW_TIMEOUT:
        TC_JPEG_DEC_LOG_E("TB: jpeg API returned : JPEGDEC_HW_TIMEOUT\n");
        break;
    case JPEGDEC_UNSUPPORTED:
        TC_JPEG_DEC_LOG_E("TB: jpeg API returned : JPEGDEC_UNSUPPORTED\n");
        break;
    case JPEGDEC_PARAM_ERROR:
        TC_JPEG_DEC_LOG_E("TB: jpeg API returned : JPEGDEC_PARAM_ERROR\n");
        break;
    case JPEGDEC_MEMFAIL:
        TC_JPEG_DEC_LOG_E("TB: jpeg API returned : JPEGDEC_MEMFAIL\n");
        break;
    case JPEGDEC_INITFAIL:
        TC_JPEG_DEC_LOG_E("TB: jpeg API returned : JPEGDEC_INITFAIL\n");
        break;
    case JPEGDEC_HW_BUS_ERROR:
        TC_JPEG_DEC_LOG_E("TB: jpeg API returned : JPEGDEC_HW_BUS_ERROR\n");
        break;
    case JPEGDEC_SYSTEM_ERROR:
        TC_JPEG_DEC_LOG_E("TB: jpeg API returned : JPEGDEC_SYSTEM_ERROR\n");
        break;
    case JPEGDEC_DWL_ERROR:
        TC_JPEG_DEC_LOG_E("TB: jpeg API returned : JPEGDEC_DWL_ERROR\n");
        break;
    ...
    case JPEGDEC_STRM_ERROR:
        TC_JPEG_DEC_LOG_E("TB: jpeg API returned : JPEGDEC_STRM_ERROR\n");
        break;
    ...
    case JPEGDEC_SLICE_MODE_UNSUPPORTED:
        ...
        break;
    default:
        TC_JPEG_DEC_LOG_E("TB: jpeg API returned unknown status\n");
        break;
    }
}
...
static u32 pp_startup(const void *decInst, u32 decType, const JpegDecImageInfo *ImgInfo, u32 out_cf)
{
...

    {
        ...
        TC_JPEG_DEC_LOG_I("pp outimg buffer %x", ppOutImg->bufferBusAddr);
            ...
            u32 out_buf_len)
{
...

    {
        ...
        {
            TC_JPEG_DEC_LOG_E("out_cf=%x, out_buf_len %d < buffer_len %d", out_cf, out_buf_len , buffer_len);
            return 1;
        }
...
startFullDecode:
    ...
    {
        TC_JPEG_DEC_LOG_I("PP INITIALIZATION FAILED\n");
        goto end;
    }
#endif
    START_SW_PERFORMANCE;
    ...
    TC_JPEG_DEC_LOG_I("PHASE 1: INIT JPEG DECODER successful\n");
    ...
    if (writeOutput)
        TC_JPEG_DEC_LOG_I("\t-File: Write output: YES: %d\n", writeOutput);
    else
        TC_JPEG_DEC_LOG_I("\t-File: Write output: NO: %d\n", writeOutput);
    TC_JPEG_DEC_LOG_I("\t-File: MbRows/slice: %d\n", jpegIn.sliceMbSet);
    TC_JPEG_DEC_LOG_I("\t-File: Buffer size: %d\n", jpegIn.bufferSize);
    TC_JPEG_DEC_LOG_I("\t-File: Stream size: %d\n", jpegIn.streamLength);
...
decode:
    ...
    if (jpegRet != JPEGDEC_OK)
    {
        ...
        {
            TC_JPEG_DEC_LOG_I("JPEGDEC_INCREASE_INPUT_BUFFER\n");
            goto end;
        }
        else
        {
            ...
            TC_JPEG_DEC_LOG_I("\n\t--------------------------------------\n");
            TC_JPEG_DEC_LOG_I("\tNOTE! IMAGE INFO WAS CHANGED!!!\n");
            TC_JPEG_DEC_LOG_I("\t--------------------------------------\n\n");
            ...
            if (mode)
            {
                ...
                {
                    TC_JPEG_DEC_LOG_I("\n\t--------------------------------------\n");
                    TC_JPEG_DEC_LOG_I("\tNOTE! THUMB INFO NOT CHANGED ==> DECODE!!!\n");
                    TC_JPEG_DEC_LOG_I("\t--------------------------------------\n\n");
                }
                else
                {
            ...
            else
            {
                ...
                {
                    TC_JPEG_DEC_LOG_I("\n\t--------------------------------------\n");
                    TC_JPEG_DEC_LOG_I("\tNOTE! FULL IMAGE INFO NOT CHANGED ==> DECODE!!!\n");
                    TC_JPEG_DEC_LOG_I("\t--------------------------------------\n\n");
                }
                else
                {
    ...
    TC_JPEG_DEC_LOG_I("PHASE 3: GET IMAGE INFO successful\n");
    ...
    do
    {
        ...
        {
            TC_JPEG_DEC_LOG_I("\t-JPEG: JPEGDEC_FRAME_READY\n");
        ...
        else if (jpegRet == JPEGDEC_SCAN_PROCESSED)
        {
            ...
            TC_JPEG_DEC_LOG_I("\t-JPEG: JPEGDEC_SCAN_PROCESSED\n");
            goto end;
        }
        else if (jpegRet == JPEGDEC_SLICE_READY)
        {
            TC_JPEG_DEC_LOG_I("\t-JPEG: JPEGDEC_SLICE_READY\n");
            goto end;
        }
        ...
        {
            TC_JPEG_DEC_LOG_I("\t-JPEG: JPEGDEC_STRM_ERROR\n");
...
error:
    ...
    if (!mode)
        ...
        TC_JPEG_DEC_LOG_I("\t-JPEG: ++++++++++ THUMBNAIL ++++++++++\n");

    TC_JPEG_DEC_LOG_I("\t-JPEG: Instance %x\n", (JpegDecContainer *) jpeg);
    TC_JPEG_DEC_LOG_I("\t-JPEG: Luma output: 0x%16x size: 0x%x\n",
                        jpegOut.outputPictureY.pVirtualAddress, GetLumaSize(&imageInfo, mode));
    TC_JPEG_DEC_LOG_I("\t-JPEG: Chroma output: 0x%16x size: 0x%x\n",
                        jpegOut.outputPictureCbCr.pVirtualAddress, GetChromaSize(&imageInfo, mode));
    TC_JPEG_DEC_LOG_I("\t-JPEG: Luma output bus: 0x%16x\n",
                        (u8 *) jpegOut.outputPictureY.busAddress);
    TC_JPEG_DEC_LOG_I("\t-JPEG: Chroma output bus: 0x%16x\n",
                        (u8 *) jpegOut.outputPictureCbCr.busAddress);

    TC_JPEG_DEC_LOG_I("PHASE 4: DECODE FRAME successful\n");
    ...
    TC_JPEG_DEC_LOG_I("\t-JPEG: PP_OUTPUT_WRITE\n");
    START_SW_PERFORMANCE;
    pp_api_wait();
    END_SW_PERFORMANCE;
#endif
    ...
    TC_JPEG_DEC_LOG_I("PHASE 6: RELEASE JPEG DECODER successful\n\n");
#if 0
...
int jpeg_decode(uint8_t *byteStrmStart, 
            uint32_t streamTotalLen, 
            const char *out_cf,
            uint8_t *out_buf,
            uint32_t out_buf_len)
...
TC_JPEG_DEC_LOG_I("\t-JPEG: DECODE DONE\n");
